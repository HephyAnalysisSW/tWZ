...
argParser.add_argument('--plot_directory', action='store', default='EFT_UL_gamma_v1')
...
if args.era == "UL2016":
    ...
    mc = [UL2018.DY, UL2018.TTLep, UL2018.TTGamma, UL2018.ZGamma]
    samples_eft = []
    ...
    if sample.name != "data":
        for i in range(Nmax):
            if abs(event.GenPart_pdgId[i]) in [11,13]:
                motherID = event.GenPart_genPartIdxMother[i]
                if motherID >= 0:
                    motherPDG = event.GenPart_pdgId[motherID]
                    if motherPDG != event.GenPart_pdgId[i]:
                        if abs(motherPDG) == 22:
                            PhotonConv = 1
                            matched_leptons_idx.append(i)

    # match to reco leptons
    for i in matched_leptons_idx:
        eta = event.GenPart_eta[i]
        phi = event.GenPart_phi[i]
        for (eta_rec, phi_rec) in [ (event.l1_eta,event.l1_phi), (event.l2_eta,event.l2_phi), (event.l3_eta,event.l3_phi)]:
            deta = eta-eta_rec
            dphi = deltaPhi(phi,phi_rec)
            dR = sqrt( deta*deta + dphi*dphi)
            if dR < 0.2:
                PhotonConv = 2

    event.PhotonConv = PhotonConv
sequence.append(fromGamma)
...
for i_mode, mode in enumerate(allModes):
    ...
    plots.append(Plot(
        name = "PhotonConv",
        texX = 'Photon conversion category', texY = 'Number of Events',
        attribute = lambda event, sample: event.PhotonConv,
        binning=[5,-0.5,4.5],
    ))
