...
def getShapesCommand(dataCard_dir, infile, outname):
    cmd = "combine INROOTFILE -M FitDiagnostics --saveShapes --saveWithUnc --numToysForShape 2000  --plots -n OUTNAME"
...
def getImpactCommands(dataCard_dir, infile, outname):
    base_cmd =  "combineTool.py -M Impacts -d INROOTFILE -m 125 "
...
for r in range(nRegions)+["combined"]:
    ...
    infile = "FakerateClosureMConly_%s_%s_13TeV_%s.root"%(args.year, str(region), args.year)
    outname = "."+infile.replace(".root", "")

    # Now put together combine command
    cmd_combine =  "combine -M MultiDimFit INROOTFILE -m 125 -n OUTNAME --verbose -1 --saveToys --saveWorkspace"
