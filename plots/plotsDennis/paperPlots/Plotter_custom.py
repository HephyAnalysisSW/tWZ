...
class Plotter:
    ...
    def addSignal(self, hist_, legendtext, color, lineStyle=1, lineWidth=2, ratioPad=1):
        hist = hist_.Clone()
    ...
    def addText(self, x, y, text, font=43, size=12, pad=1):
        ...
        latex.SetX(x)
        latex.SetY(y)
        self.__latexTexts.append( (latex,pad) )
    ...
    ############################################################################
    # Private, create the ratio plot
    def __getRatioLine(self, name="line"):
        line = ROOT.TH1F(name, name, self.__Nbins, arr.array('d',self.__binning))
        for i in range(self.__Nbins):
            bin=i+1
    ...
    def __getCMS(self):
        ...
        if self.drawRatio: cmstext.SetTextSize(0.10)
    ...
    def __setRatioDrawOptions(self, ratio, isData=False, drawXaxis=True, xticklength=0.07, yticklength=0.07, noytitle=False):
        ...
        ratio.GetXaxis().SetTickLength(xticklength)
        ratio.GetYaxis().SetTickLength(yticklength)
        ...
        if not drawXaxis:
            ...
            ratio.GetXaxis().SetTitleOffset(0.0)
    ...
    def draw(self):
        ...
        pady1 = 0.466666+space if self.drawRatio else 0.0
        pad1 = ROOT.TPad("pad1", "pad1", 0, pady1, 1, 1.0)
        if self.drawRatio: pad1.SetBottomMargin(0.08)
        ...
        if self.drawRatio:
            pad2 = ROOT.TPad("pad2", "pad2", 0, 0.0, 1, 0.24)
            pad2.SetLeftMargin(self.__MarginLeft)
            pad2.SetRightMargin(self.__MarginRight)
            pad2.SetTopMargin(0)
            pad2.SetBottomMargin(self.__MarginBottom+0.05)
            pad2.Draw()

            pad3 = ROOT.TPad("pad3", "pad3", 0.0, 0.24+space, 1, 0.353+space)
            pad3.SetLeftMargin(self.__MarginLeft)
            pad3.SetRightMargin(self.__MarginRight)
            pad3.SetTopMargin(0)
            pad3.SetBottomMargin(0)
            pad3.Draw()

            pad4 = ROOT.TPad("pad4", "pad4", 0.0, 0.353+space*2, 1, 0.466666+space*2)
            pad4.SetLeftMargin(self.__MarginLeft)
            pad4.SetRightMargin(self.__MarginRight)
            pad4.SetTopMargin(0)
        ...
        if self.__hasData:
            self.__legend.AddEntry(self.__data["hist"], self.__data["name"], "pe")
        if self.__hasBackground:
            self.__buildStack()
        ...
        if self.drawRatio:
            ...
            ratioline_pad2.Draw("HIST")
            if self.debug: print("Draw ratio error band")
            ratio_uncert_pad2 = self.__getRatioUncert(self.__errorhist)
            self.__setUncertDrawOptions(ratio_uncert_pad2)
            ratio_uncert_pad2.Draw("E2 SAME")
            if self.__hasSignal:
            ...
            for (text,pad) in self.__latexTexts:
                if pad == 2:
                    text.Draw()
            ...
            pad3.cd()
            if self.debug: print("Draw ratio line")
            ratioline_pad3 = self.__getRatioLine("line3")
            self.__setRatioDrawOptions(ratioline_pad3, drawXaxis=False, xticklength=0.18, yticklength=0.025)
            ratioline_pad3.SetFillColor(0)
            ratioline_pad3.SetLineColor(15)
            ratioline_pad3.SetLineWidth(2)
            ratioline_pad3.Draw("HIST")
            if self.debug: print("Draw ratio error band")
            ratio_uncert_pad3 = self.__getRatioUncert(self.__errorhist)
            self.__setUncertDrawOptions(ratio_uncert_pad3)
            ratio_uncert_pad3.Draw("E2 SAME")
            if self.__hasSignal:
                if self.debug: print("Draw signal ratios")
                ratios_sig_pad3 = []
                for sig in self.__signals:
                    if sig["ratioPad"] == 2:
                        ratios_sig_pad3.append(self.__getRatio(sig["hist"], self.__bkgtotal, sig["color"], sig["linestyle"], sig["linewidth"]) )
                for r in ratios_sig_pad3:
            ...
            ratioline_pad4.Draw("HIST")
            if self.debug: print("Draw ratio error band")
            ratio_uncert_pad4 = self.__getRatioUncert(self.__errorhist)
            self.__setUncertDrawOptions(ratio_uncert_pad4)
            ratio_uncert_pad4.Draw("E2 SAME")
            if self.__hasSignal:
                if self.debug: print("Draw signal ratios")
                ratios_sig_pad4 = []
                for sig in self.__signals:
