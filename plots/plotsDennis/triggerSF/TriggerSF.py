...
argParser.add_argument('--plot_directory', action='store', default='TriggerSF_v1')
...
from tWZ.samples.nanoTuples_ULRunII_nanoAODv9_postProcessed_triggerSF import *

if args.era == "UL2016":
    mc = [UL2016.ZZ_powheg, UL2016.WZTo3LNu, UL2016.WZTo3LNu_powheg, UL2016.TTZ]
    samples_eft = []
elif args.era == "UL2016preVFP":
    mc = [UL2016preVFP.ZZ_powheg, UL2016preVFP.WZTo3LNu, UL2016preVFP.WZTo3LNu_powheg, UL2016preVFP.TTZ]
    samples_eft = []
elif args.era == "UL2017":
    mc = [UL2017.ZZ_powheg, UL2017.WZTo3LNu, UL2017.WZTo3LNu_powheg, UL2017.TTZ]
    samples_eft = []
elif args.era == "UL2018":
    mc = [UL2018.ZZ_powheg, UL2018.WZTo3LNu, UL2018.WZTo3LNu_powheg, UL2018.TTZ]
    samples_eft = []
...
for i_mode, mode in enumerate(allModes):
    ...
    plots.append(Plot(
        ...
        attribute = lambda event, sample: event.Z1_pt,
        binning=[50, 0, 1000],
    ))
    ...
    # Only events passing trigger

    plots.append(Plot(
      name = 'yield_passTrigger', texX = '', texY = 'Number of Events',
      attribute = lambda event, sample: 0.5 + i_mode if event.triggerDecision == 1 else float('NaN'),
      binning=[4, 0, 4],
    ))

    plots.append(Plot(
        name = "Z1_pt_passTrigger",
        texX = 'p_{T}(Z_{1}) (GeV)', texY = 'Number of Events / 20 GeV',
        attribute = lambda event, sample: event.Z1_pt if event.triggerDecision == 1 else float('NaN'),
        binning=[50, 0, 1000],
    ))


    plots.append(Plot(
        name = "l1_pt_passTrigger",
        texX = 'Leading lepton p_{T} (GeV)', texY = 'Number of Events / 10 GeV',
        addOverFlowBin='both',
        attribute = lambda event, sample: event.l1_pt if event.triggerDecision == 1 else float('NaN'),
        binning=[40, 0, 400],
    ))

    plots.append(Plot(
        name = "l2_pt_passTrigger",
        texX = 'Subleading lepton p_{T} (GeV)', texY = 'Number of Events / 10 GeV',
        addOverFlowBin='both',
        attribute = lambda event, sample: event.l2_pt if event.triggerDecision == 1 else float('NaN'),
        binning=[40, 0, 400],
    ))

    plots.append(Plot(
        name = "l3_pt_passTrigger",
        texX = 'Trailing lepton p_{T} (GeV)', texY = 'Number of Events / 10 GeV',
        addOverFlowBin='both',
        attribute = lambda event, sample: event.l3_pt if event.triggerDecision == 1 else float('NaN'),
        binning=[40, 0, 400],
    ))
    ...
    for plot in plots:
        if plot.name in ["yield", "yield_passTrigger"]:
...
plots_root  = ["Z1_pt", "l1_pt", "l2_pt", "l3_pt", "yield"]
plots_root += ["Z1_pt_passTrigger", "l1_pt_passTrigger", "l2_pt_passTrigger", "l3_pt_passTrigger", "yield_passTrigger"]
